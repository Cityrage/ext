...
namespace math {
    ...
    Matrix3 Matrix3::from_pitch(float pitch) {
        float rad = pitch * (3.14159265358979323846f / 180.0f);
        float c = cos(rad);
        float s = sin(rad);

        Matrix3 result{};
        result.data[0] = 1; result.data[1] = 0; result.data[2] = 0;
        result.data[3] = 0; result.data[4] = c; result.data[5] = -s;
        result.data[6] = 0; result.data[7] = s; result.data[8] = c;
        return result;
    }

...
template <typename T>
bool read_array(uint64_t address, T* buffer, size_t count) {
    HANDLE current_handle = mem::roblox_h.load();
    ...
    if (mem::use_nt_functions.load() && mem::NtReadVirtualMemory) {
        ULONG nt_bytes_read = 0;
        NTSTATUS status = mem::NtReadVirtualMemory(current_handle, (PVOID)address, buffer, (ULONG)total_size, &nt_bytes_read);
        return (status == 0 && nt_bytes_read == total_size);
    }
    else {
        SIZE_T bytes_read = 0;
        return ReadProcessMemory(current_handle, (PVOID)address, buffer, total_size, &bytes_read) != FALSE;
    }
}

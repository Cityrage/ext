...
// Forward declarations for overlay symbols referenced in this translation unit
namespace overlay {
    extern bool visible;
}
...
void hooks::cache() {
    while (true) {
        ...
        static int wallcheck_refresh_counter = 0;
        if (wallcheck_refresh_counter++ % 10 == 0) { // Refresh wallcheck cache every 10 iterations (~100ms)

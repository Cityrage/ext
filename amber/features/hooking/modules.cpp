...
public:
    ...
    static void InitAvatarManager(ID3D11Device* device, ID3D11DeviceContext* context) {
        if (!avatar_manager) {
            avatar_manager = std::make_unique<AvatarManager>(device, context);
        }
    }
...
public:
    ...
    void launchThreads() {
        ...
        try {
            ...
            hook_threads.emplace_back([]() {
                logger.CustomLog(FOREGROUND_BLUE | FOREGROUND_INTENSITY, "THREAD", "Starting nojumpcooldown thread");
                hooks::nojumpcooldown();
                });

            hook_threads.emplace_back([]() {
                logger.CustomLog(FOREGROUND_BLUE | FOREGROUND_INTENSITY, "THREAD", "Starting macro thread");
                hooks::macro();
                });
...
namespace overlay {
    AvatarManager* get_avatar_manager() {
        return ThreadSafePlayerCache::GetAvatarManager();
    }
    
    void init_avatar_manager(void* device, void* context) {
        ThreadSafePlayerCache::InitAvatarManager((ID3D11Device*)device, (ID3D11DeviceContext*)context);
    }
}

...
static bool is_in_target_list(const CachedPlayerData& player) {
    if (globals::visuals::target_only_list.empty()) return true; // No filter when list is empty

    std::string uname = player.player.name;
    std::string dname = player.player.displayname;
    std::transform(uname.begin(), uname.end(), uname.begin(), [](unsigned char c) { return to_lower_ascii(c); });
    std::transform(dname.begin(), dname.end(), dname.begin(), [](unsigned char c) { return to_lower_ascii(c); });

    for (const auto& entry : globals::visuals::target_only_list) {
        std::string lowered = entry;
        std::transform(lowered.begin(), lowered.end(), lowered.begin(), [](unsigned char c) { return to_lower_ascii(c); });
        if (lowered == uname || lowered == dname) return true;
    }
    return false;
}
...
// Global variables to store original fog settings for restoration
float original_fog_start = 0.0f;
float original_fog_end = 0.0f;
math::Vector3 original_fog_color = { 0.0f, 0.0f, 0.0f };
bool original_fog_settings_saved = false;
std::mutex fog_mutex;
...
void render_box_enhanced(const math::Vector2& top_left, const math::Vector2& bottom_right, ImColor color) {
    ...
    if ((*globals::visuals::box_overlay_flags)[1]) {
        ImVec4 glow_color_vec = ImGui::ColorConvertU32ToFloat4(ImGui::ColorConvert(globals::visuals::glowcolor));
        glow_color_vec.w *= globals::visuals::glow_opacity; // Apply opacity
        ImColor glow_color = ImGui::ColorConvertFloat4ToU32(glow_color_vec);
        globals::instances::draw->AddShadowRect(
            ImVec2(top_left.x, top_left.y),
            ImVec2(bottom_right.x, bottom_right.y),
            glow_color, globals::visuals::glow_size, ImVec2(0, 0), ImDrawFlags_ShadowCutOutShapeBackground, 0.0f
        );
    }

    // Handle Fill
    if ((*globals::visuals::box_overlay_flags)[2]) {
        ImColor boxfill = ImGui::ColorConvert(globals::visuals::boxfillcolor);
    ...
    // 0 = Corners, 1 = Bounding (Matching UI order)
    if (globals::visuals::boxtype == 1) { // Bounding
        if ((*globals::visuals::box_overlay_flags)[0]) { // Outline
            globals::instances::draw->AddRect(
                ImVec2(top_left.x - 1, top_left.y - 1),
                ImVec2(bottom_right.x + 1, bottom_right.y + 1),
                ImColor(0, 0, 0, 255), 0.0f, 0, 1.0f
            );
            globals::instances::draw->AddRect(
                ImVec2(top_left.x + 1, top_left.y + 1),
                ImVec2(bottom_right.x - 1, bottom_right.y - 1),
    ...
    else if (globals::visuals::boxtype == 0) { // Corners
        ...
        if ((*globals::visuals::box_overlay_flags)[0]) { // Outline
            ...
            auto draw_corner_outline = [&](ImVec2 p1, ImVec2 p2, ImVec2 p3) {
                globals::instances::draw->AddLine(ImVec2(p1.x - 1, p1.y), ImVec2(p2.x + 1, p2.y), outline_color, out_t + 2.0f);
                globals::instances::draw->AddLine(ImVec2(p1.x, p1.y - 1), ImVec2(p3.x, p3.y + 1), outline_color, out_t + 2.0f);
            };

            // Needs a cleaner way to draw corner outlines, but for now simple offsets
            globals::instances::draw->AddLine(ImVec2(top_left.x - 1, top_left.y), ImVec2(top_left.x + corner_size + 1, top_left.y), outline_color, 3.0f);
            globals::instances::draw->AddLine(ImVec2(top_left.x, top_left.y - 1), ImVec2(top_left.x, top_left.y + corner_size + 1), outline_color, 3.0f);

            globals::instances::draw->AddLine(ImVec2(bottom_right.x + 1, top_left.y), ImVec2(bottom_right.x - corner_size - 1, top_left.y), outline_color, 3.0f);
            globals::instances::draw->AddLine(ImVec2(bottom_right.x, top_left.y - 1), ImVec2(bottom_right.x, top_left.y + corner_size + 1), outline_color, 3.0f);

            globals::instances::draw->AddLine(ImVec2(top_left.x - 1, bottom_right.y), ImVec2(top_left.x + corner_size + 1, bottom_right.y), outline_color, 3.0f);
            globals::instances::draw->AddLine(ImVec2(top_left.x, bottom_right.y + 1), ImVec2(top_left.x, bottom_right.y - corner_size - 1), outline_color, 3.0f);

            globals::instances::draw->AddLine(ImVec2(bottom_right.x + 1, bottom_right.y), ImVec2(bottom_right.x - corner_size - 1, bottom_right.y), outline_color, 3.0f);
            globals::instances::draw->AddLine(ImVec2(bottom_right.x, bottom_right.y + 1), ImVec2(bottom_right.x, bottom_right.y - corner_size - 1), outline_color, 3.0f);
        }
...
void render_health_bar_enhanced(const math::Vector2& top_left, const math::Vector2& bottom_right, float current_health, float max_health, ImColor color) {
    if (!globals::visuals::health) return;

    float health_percent = std::clamp(current_health / max_health, 0.0f, 1.0f);
    float box_height = bottom_right.y - top_left.y;
    float bar_width = 1.3f; // Made health bar thicker
    
    // Calculate bar position based on health_bar_position setting
    // 0 = Left, 1 = Right
    float bar_x;
    if (globals::visuals::health_bar_position == 1) { // Right
        bar_x = bottom_right.x + 4.0f;
    } else { // Left (default)
        bar_x = top_left.x - bar_width - 4.0f;
    }

    // Health bar glow rendering
    if (globals::visuals::enable_health_glow) {
        ImU32 GlowColor = ImGui::ColorConvertFloat4ToU32(ImVec4(
            globals::visuals::healthglowcolor[0], 
            globals::visuals::healthglowcolor[1], 
            globals::visuals::healthglowcolor[2], 
            globals::visuals::health_glow_opacity
        ));
        ...
        globals::instances::draw->AddShadowRect(
            ...
            GlowColor, 
            globals::visuals::health_glow_size, 
            ImVec2(0, 0), 
    ...
    // Outline (render if enabled)
    if (globals::visuals::health_bar_outline) {
        ImColor outline_color = ImColor(0, 0, 0, 255);
        globals::instances::draw->AddRectFilled(
    ...
    ImColor color_high = ImGui::ColorConvert(globals::visuals::healthbarcolor);
    ImColor color_low = ImGui::ColorConvert(globals::visuals::healthbarcolor1);
    float fill_height = box_height * health_percent;

    if (globals::visuals::health_bar_gradient) { // Gradient
    ...
    else { // Standard - interpolate between colors based on health
        // Interpolate between color_low (low health) and color_high (high health)
        ImVec4 low_vec = ImGui::ColorConvertU32ToFloat4(color_low);
        ImVec4 high_vec = ImGui::ColorConvertU32ToFloat4(color_high);
        
        float r = low_vec.x + (high_vec.x - low_vec.x) * health_percent;
        float g = low_vec.y + (high_vec.y - low_vec.y) * health_percent;
        float b = low_vec.z + (high_vec.z - low_vec.z) * health_percent;
        float a = low_vec.w + (high_vec.w - low_vec.w) * health_percent;
        
        ImColor interpolated_color = ImGui::ColorConvertFloat4ToU32(ImVec4(r, g, b, a));
...
void render_name_enhanced(const std::string& username, const std::string& display_name, const math::Vector2& top_left, const math::Vector2& bottom_right) {
    if (!globals::visuals::name) return;
    
    std::string label = (globals::visuals::nametype == 0) ? username : display_name;
    ...
    draw_text_with_shadow_enhanced(13.0f, ImVec2(center_x - text_size.x * 0.5f, y_pos), ImGui::ColorConvert(globals::visuals::namecolor), label.c_str());
}
...
void render_snapline_enhanced(const math::Vector2& screen_pos, SnaplineType type, bool is_locked) {
    ...
    if (globals::visuals::snaplineoverlaytype == 0) { // Straight
        ...
        // Control point for the curve (downward arc)
        ImVec2 control(midX, midY + 250.0f);
        ...
        // Second pass: draw main colored line
        for (int i = 0; i < segments; i++)
        {
            float t0 = i / (float)segments;
            float t1 = (i + 1) / (float)segments;
    ...
    while (visuals::threadRunning) {
        try {
            ...
            for (auto& player : all_entities) {
                ...
                // Filter out teammates when teamcheck is on
                if (((*globals::visuals::visuals_flags)[0] != 0) || globals::combat::teamcheck) {
                ...
                try {
                    ...
                    // Populate name/display_name based on nametype
                    std::string username = player.name;
                    std::string display = displaynames::get_best_display(player);
                    data.name = username;
                    data.display_name = display;
                    
                    // The actual label to show depends on the setting
                    std::string label = (globals::visuals::nametype == 0) ? username : display;
...
void visuals::apply_fog_settings() {
    ...
    if (globals::visuals::fog_enabled) {
        ...
        // globals::instances::lighting.set_fog_color(math::Vector3(globals::visuals::fog_color[0], globals::visuals::fog_color[1], globals::visuals::fog_color[2]));
    }
    else {
        if (original_fog_settings_saved) {
            globals::instances::lighting.write_fogstart(original_fog_start);
...
void utils::CacheChamsPoly() {
    while (true) {
        ...
        for (roblox::player& player : playerlist) {
            ...
            // Segmented Hull for Highlight (Type 2) 
            // Groups: Body, Left Arm, Right Arm, Left Leg, Right Leg
            if (globals::visuals::chamstype == 2) {
                // Determine parts for each group based on rig type
                std::vector<std::vector<roblox::instance>> groups;
                if (player.rigtype == 0) { // R6
                    groups = {
                        { player.head, player.uppertorso },           // Body (Torso serves as both upper/lower)
                        { player.lefthand },                          // Left Arm (Hand/Arm are one part usually or we just use what we have)
                        { player.righthand },                         // Right Arm
                        { player.leftfoot },                          // Left Leg
                        { player.rightfoot }                          // Right Leg
                    };
                    // Note: R6 parts mapping in previous switch was: 
                    // uppertorso -> "Torso", 
                    // lefthand -> "Left Arm", righthand -> "Right Arm",
                    // leftfoot -> "Left Leg", rightfoot -> "Right Leg"
                    // The 'parts_chams' vector above didn't preserve the structure, so we need to rebuild groups manually or use the player pointer directly.
                    // However, we are inside a loop over 'playerlist'. We can access 'player' directly.
                    // Let's redefine groups using the player object directly for clarity and correctness.
                } else { // R15
                    groups = {
                        { player.head, player.uppertorso, player.lowertorso }, // Body
                        { player.leftupperarm, player.leftlowerarm, player.lefthand }, // Left Arm
                        { player.rightupperarm, player.rightlowerarm, player.righthand }, // Right Arm
                        { player.leftupperleg, player.leftlowerleg, player.leftfoot }, // Left Leg
                        { player.rightupperleg, player.rightlowerleg, player.rightfoot } // Right Leg
                    };
                }

                // But wait, for R6 the previous switch (lines 960-969) mapped:
                // uppertorso -> Torso
                // lefthand -> Left Arm
                // etc.
                // So the player struct has the correct parts. We should just use them.
                
                // Re-define groups to ensure we cover all parts correctly
                 std::vector<std::vector<roblox::instance>> part_groups;
                 if (player.rigtype == 0) {
                     part_groups = {
                     ...
                     part_groups = {
                         { player.head, player.uppertorso, player.lowertorso },
                         { player.leftupperarm, player.leftlowerarm, player.lefthand },
                         { player.rightupperarm, player.rightlowerarm, player.righthand },
                         { player.leftupperleg, player.leftlowerleg, player.leftfoot },
                         { player.rightupperleg, player.rightlowerleg, player.rightfoot }
                     };
                 }

                 for (auto& group : part_groups) {
                     std::vector<ImVec2> group_projected;
                     ...
                     for (auto& part : group) {
                         ...
                         try {
                            ...
                            for (auto& vertex : cube_vertices) {
                                ...
                                group_projected.emplace_back(ImVec2(screen.x, screen.y));
                                group_depth += vertex.z;
                                group_count++;
                            }
                         } catch (...) { continue; }
                     }

                     if (group_projected.size() >= 3) {
                         std::sort(group_projected.begin(), group_projected.end(), [](const ImVec2& a, const ImVec2& b) {
                         ...
                         for (int i = static_cast<int>(group_projected.size()) - 1; i >= 0; --i) {
                            auto& p = group_projected[i];
                         ...
                         if (hull.size() >= 3) {
                            float avg_depth = group_count > 0 ? group_depth / group_count : 0.0f;
                            temp_polygons.emplace_back(Polygon_t{ hull, avg_depth });
                         }
                     }
                 }
            }  
            else { 
                // Existing Per-Part Logic for Outline/Fill
...
void render_chams_enhanced() {
    ...
    // 0 = Outline, 1 = Fill, 2 = Highlight
    bool draw_outline = (globals::visuals::chamstype == 0);
    bool draw_fill = (globals::visuals::chamstype == 1);
    bool draw_highlight = (globals::visuals::chamstype == 2);

    // If Highlight is selected, overwrite the outline color with chamscolor1
    if (draw_highlight) {
         outlineColor = ImColor(
            globals::visuals::chamscolor1[0],
            globals::visuals::chamscolor1[1],
            globals::visuals::chamscolor1[2],
            globals::visuals::chamscolor1[3]
        );
    }
    ...
    for (const auto& polygon : polygons_to_draw) {
        ...
        if (draw_outline || draw_highlight) {
    ...
    if (globals::visuals::sonar_detect_players) {
...

        {
            ...
            for (const auto& player : visuals::espData.cachedPlayers) {
                ...
                if (globals::visuals::target_only_esp && !is_in_target_list(player)) continue;
...
void visuals::run() {
    ...
    // Draw FOV circles for aimbot and silent aim (before visuals check so they always show)
    if (globals::instances::draw && globals::focused && keybind::is_roblox_focused()) {
        HWND robloxWindow = FindWindowA(0, "Roblox");
        if (robloxWindow) {
            POINT cursor_pos;
            if (GetCursorPos(&cursor_pos) && ScreenToClient(robloxWindow, &cursor_pos)) {
                ImVec2 mousepos = ImVec2((float)cursor_pos.x, (float)cursor_pos.y);
                ImDrawList* drawbglist = globals::instances::draw;
                
                // Helper function for drawing rotated N-gons
                ...
                // Draw Aimbot FOV if enabled
                if (globals::combat::aimbot && globals::combat::usefov) {
                    float rotation_angle = globals::combat::spin_fov_aimbot ? (current_time * globals::combat::spin_fov_aimbot_speed) : 0.0f;
                    ...
                    case 0: // Circle
                        ...
                        drawbglist->AddCircle(mousepos, globals::combat::fovsize, fov_color_with_transparency, 0, globals::combat::fovthickness);
                        break;
                    case 1: // Square
                ...
                // Draw Silent Aim FOV if enabled
                if (globals::combat::silentaim && globals::combat::silentaimfov) {
                    float rotation_angle = globals::combat::spin_fov_silentaim ? (current_time * globals::combat::spin_fov_silentaim_speed) : 0.0f;
                    ...
                    case 0: // Circle
                        ...
                        drawbglist->AddCircle(mousepos, globals::combat::silentaimfovsize, silent_aim_fov_color_with_transparency, 0, globals::combat::silentaimfovthickness);
                        break;
                    case 1: // Square
    ...
    for (auto& player : visuals::espData.cachedPlayers) {
        ...
        if (globals::visuals::target_only_esp && !is_in_target_list(player)) {
            continue; // Respect target-only filter (matches username or display, case-insensitive)
        }
        ...
        bool is_locked_target = globals::visuals::lockedesp &&
            globals::instances::cachedtarget.main.address != 0 &&
            globals::instances::cachedtarget.main.address == player.player.main.address;

        if (globals::visuals::boxes) {
            ImColor base_color = ImGui::ColorConvert(globals::visuals::boxcolors); // Visible
            if ((*globals::visuals::visuals_flags)[3]) { // WallCheck flag
                if (!wallcheck::can_see(visuals::espData.localPos, player.position)) {
                    base_color = ImGui::ColorConvert(globals::visuals::boxcolors1); // Occluded
                }
            }
            
            if (is_locked_target)
                base_color = ImGui::ColorConvert(globals::visuals::lockedespcolor);

            render_box_enhanced(top_left, bottom_right, base_color);
        }

        if (globals::visuals::name) {
            render_name_enhanced(player.name, player.display_name, top_left, bottom_right);
        }
        ...
        if (globals::visuals::snapline) {
            render_snapline_enhanced(screen_check, static_cast<SnaplineType>(globals::visuals::snaplinetype), is_locked_target);
        }
        ...
        if (globals::visuals::health) {
            ...
            render_health_bar_enhanced(top_left, bottom_right, current_health, max_health, ImGui::ColorConvert(globals::visuals::healthbarcolor));
        }
    ...
    else if (original_fog_settings_saved) {

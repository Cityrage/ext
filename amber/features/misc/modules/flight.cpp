...
void hooks::flight() {
	...
	while (true) {
		...
		static math::Vector3 fly_pos = { 0.0f, 0.0f, 0.0f };
		...
		if (globals::misc::flight) {
			if (!was_enabled) {
				fly_pos = globals::instances::lp.hrp.get_pos();
				was_enabled = true;
				aircheck = false;
			}
			...
			if (globals::misc::flightkeybind.enabled) {
				...
				case 0:
					...
					if (aircheck == false) {
						hrp.write_position(fly_pos);
						hrp.write_velocity({ 0.0f, 0.0f, 0.0f });
					}
					if (aircheck == true) {
						for (int i = 0; i < 4500; i++) {
							fly_pos = fly_pos + (direction * (globals::misc::flightvalue / 25000));
							hrp.write_position(fly_pos);
							hrp.write_velocity({ 0.0f, 0.0f, 0.0f });
				...
				case 1: {
					...
					if (aircheck == false) {
						hrp.write_velocity((fly_pos - hrp.get_pos()) * 100.0f);
					}
					if (aircheck == true) {
						for (int i = 0; i < 1500; i++) {
							fly_pos = fly_pos + (direction * (globals::misc::flightvalue / 10000.0f));
							hrp.write_velocity((fly_pos - hrp.get_pos()) * 100.0f);
						}
					}
					break;

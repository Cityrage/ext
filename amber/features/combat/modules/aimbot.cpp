...
roblox::player gettargetclosesttomouse(bool isAimbotActiveParam, bool isSilentAimbotActiveParam) {
    ...
    std::vector<roblox::instance> current_bots;
    {
        ...
        current_bots = globals::instances::bots;
    }

    for (auto bot_head : current_bots) {
...
void hooks::aimbot() {
    ...
    while (true) {
        ...
        if (globals::misc::rotate360keybind.is_pressed() && globals::misc::rotate360 && !rotation_started) {
            ...
            globals::misc::camera_rotation_yaw = 0.0f;
            globals::misc::camera_rotation_pitch = 0.0f;
            rotation_steps_completed = 0;
            globals::misc::original_camera_rotation = globals::instances::camera.getRot();
        }

        if (rotating_once) {
            roblox::camera camera_instance = globals::instances::camera;
            
            globals::misc::camera_rotation_yaw += globals::misc::rotate360_speed;
            globals::misc::camera_rotation_pitch += globals::misc::rotate360_vspeed;
            rotation_steps_completed++;

            // Use the maximum of the two speeds to determine total steps for a full cycle (or just use horizontal as primary)
            float max_speed = std::max(abs(globals::misc::rotate360_speed), abs(globals::misc::rotate360_vspeed));
            if (max_speed < 0.1f) max_speed = globals::misc::rotate360_speed; // Fallback
            
            const int dynamic_total_rotation_steps = static_cast<int>(360.0f / (max_speed > 0.1f ? max_speed : 10.0f));
            ...
            if (rotation_steps_completed >= dynamic_total_rotation_steps) {
                ...
                rotation_steps_completed = 0;
                camera_instance.writeRot(globals::misc::original_camera_rotation);
            } else {
                math::Matrix3 yawRotation = math::Matrix3::from_yaw(globals::misc::camera_rotation_yaw);
                math::Matrix3 pitchRotation = math::Matrix3::from_pitch(globals::misc::camera_rotation_pitch);
                math::Matrix3 combinedRotation = yawRotation * pitchRotation;
                math::Matrix3 newRotation = globals::misc::original_camera_rotation * combinedRotation;
        ...
        if (currentStickyAim && foundTarget) { // If sticky aim is on and we have a target
            ...
            if (!target.main.is_valid() || !target.head.is_valid()) {
                ...
                globals::target_was_knocked = true; // Block retargeting while key is held
            } else {
                bool isAlive = true;
                if (globals::instances::gamename == "Arsenal") {
                    try {
                    ...
                    if (isAlive && target.health <= 0) isAlive = false;
                    // Release lock if player falls into the void (standard Arsenal behavior)
                    if (isAlive && target.head.is_valid() && target.head.get_pos().y < -50.0f) isAlive = false;
                } else {
...
// Helper to get random index
            ...
            if (isAimbotActive) {
                ...
                if (globals::combat::aimbot_closest_part) {
                   ...
                   // If no parts selected? Fallback to Torso or Head? Or nothing?
                   ...
                   for (int i = 0; i < 14; ++i) {
                       if ((*aimpart_to_use)[i]) { // Only enabled parts
                           roblox::instance part = target.instance.findfirstchild(globals::combat::hit_parts[i]);
                           if (!part.address && target.rigtype == 1) {
                               if (i == 3) part = target.instance.findfirstchild("UpperTorso");
                               else if (i == 4) part = target.instance.findfirstchild("LeftUpperLeg");
                   ...
                   if (!valid_parts.empty()) {
                       ...
                       closest_dist = 0; // Distance irrelevant for random
                   } else {
                       current_random_idx = -1;
                       // Fallback if nothing selected? Maybe default to Head/Torso just to work
                       if (!is_specific_part_selected) { // If user unselected EVERYTHING
                           // Check Head
                            roblox::instance part = target.instance.findfirstchild("Head");
                            if (part.address) {
                                targetPart = part;
                                selected_idx = 0;
                                selected_name = "Head";
                ...
                // Logic for Silent Aim Target Selection (when aimbot is NOT active)
                int selected_idx = -1;
                std::string selected_name;
                bool is_specific_part_selected = false;
                for (int i = 0; i < 14; ++i) {
                ...
                if (globals::combat::silent_closest_part) {
                    ...
                    for (int i = 0; i < 14; ++i) {
                        ...
                        if (part.address) {
                            Vector2 screen_pos = roblox::worldtoscreen(part.get_pos());
                            if (screen_pos.x != -1) {
                                // Silent aim usually prioritizes closest to mouse
                                POINT point;
                                GetCursorPos(&point);
                                ScreenToClient(robloxWindow, &point);
                                float dist = sqrt(pow(screen_pos.x - point.x, 2) + pow(screen_pos.y - point.y, 2));
                   ...
                   // Randomize among SELECTED enabled parts
                   static std::chrono::steady_clock::time_point last_switch_time = std::chrono::steady_clock::now();
                   static int current_random_idx = -1;
                   auto now = std::chrono::steady_clock::now();
        ...
        if (isAimbotActive && target.head.is_valid()) {
            ...
            // Respect silent sticky aim: keep the locked target instead of re-picking every frame
            roblox::player silent_cached = {};
            if (currentStickyAim && foundTarget && target.head.is_valid()) {
                silent_cached = target;
            } else if (silent_target.head.is_valid()) {
                silent_cached = silent_target;
            } else {
                silent_cached = gettargetclosesttomouse(false, true);
            }

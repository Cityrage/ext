...
void monitorRobloxProcess() {
    ...
    while (true) {
            ...
            if (Process32First(snapshot, &pe32)) {
                do {
                    if (_stricmp(pe32.szExeFile, "RobloxPlayerBeta.exe") == 0 || 
                        _stricmp(pe32.szExeFile, "Windows10Universal.exe") == 0) {
                        is_running = true;
                        found_once = true; // Mark as found so we don't exit before it starts
                        break;
                    }
                } while (Process32Next(snapshot, &pe32));
            }
        ...
        // Only exit if we found it once and now it's gone.
        // This handles the case where cheat starts before Roblox.
        if (found_once && !is_running) {
             // Force kill own process
             TerminateProcess(GetCurrentProcess(), 0);
        }

        // Sleep 100ms for faster response (0.1s max delay)
...
bool performAuthentication() {
    ...
    for (;;) {
        ...
        // Bigger ASCII banner for AMBER (5 lines), animated reveal, centered vertically without scrolling
        const char* banner[] = {
            "  _        _     ___   _   _ ",
            " | |      / \\   |_ _| | \\ | |",
            " | |     / _ \\   | |  |  \\| |",
...
int jsdlfhjsdlkfhjssdfjsdlfjhlsdjfsdjflkjsdlfjslghdshga() {
    auto mainInitFunction = []() -> int {
        using namespace AMBER::misc; // Bring AMBER::misc into scope
        try {
            ...
            CreateMutexA(0, FALSE, "Local\\AmberInstancelock");
            ...
            // Start Roblox process monitor for auto-close
            std::thread monitorThread(monitorRobloxProcess);

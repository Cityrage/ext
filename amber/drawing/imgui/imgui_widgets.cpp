...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (is_visible)
    {
        ...
        {
            ImVec2 pad(ImMax(1.0f, IM_TRUNC(square_sz / 3.6f)), ImMax(1.0f, IM_TRUNC(square_sz / 3.6f)));
            float r = ImGui::GetStyleColorVec4(ImGuiCol_CheckMark).x * 255.0f;
            float g = ImGui::GetStyleColorVec4(ImGuiCol_CheckMark).y * 255.0f;
            float b = ImGui::GetStyleColorVec4(ImGuiCol_CheckMark).z * 255.0f;
            ImColor OtherCOlor((int)(r)-31, (int)(g)-33, (int)(b)-35); //171, 143, 205
            window->DrawList->AddRectFilledMultiColor(check_bb.Min + ImVec2(1, 1), check_bb.Max - ImVec2(1, 1), check_col, check_col, OtherCOlor, OtherCOlor);
        }
    }
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    if (!(flags & ImGuiComboFlags_NoPreview)) {
        // Base and darker colors as ImVec4
        ImVec4 topColor = ImGui::GetStyleColorVec4(ImGuiCol_WindowBg);
        ImVec4 bottomColor;
        bottomColor.x = (topColor.x + 0.04f > 1.0f) ? 1.0f : topColor.x + 0.04f;
        bottomColor.y = (topColor.y + 0.04f > 1.0f) ? 1.0f : topColor.y + 0.04f;
        bottomColor.z = (topColor.z + 0.04f > 1.0f) ? 1.0f : topColor.z + 0.04f;
        bottomColor.w = topColor.w;
        ...
        // Border color depending on popup state
        ImVec4 borderColorVec = ImGui::GetStyleColorVec4(ImGuiCol_Border);

        window->DrawList->AddRect(
            bb.Min,
            ImVec2(value_x2, bb.Max.y),
            GetColorU32(borderColorVec),
            0.0f,
            0,
            1.0f
        );
    ...
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        ...
        ImU32 text_col;
        if (hovered) text_col = GetColorU32(ImGuiCol_CheckMark); else text_col = popup_open ? GetColorU32(ImGuiCol_CheckMark) : GetColorU32(ImGuiCol_Text);

        if (popup_open)
            window->DrawList->AddText(ImVec2(value_x2 - 13, bb.Min.y + style.FramePadding.y), text_col, "-");
        else
            window->DrawList->AddText(ImVec2(value_x2 - 15, bb.Min.y + style.FramePadding.y), text_col, "+");
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (grab_bb.Max.x > grab_bb.Min.x)
    {
        ...
        window->DrawList->AddRectFilledMultiColor(frame_bb.Min + ImVec2(2,2), grab_bb.Max - ImVec2(2,2), check_col, check_col, OtherCOlor, OtherCOlor);
    }

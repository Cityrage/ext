@echo off
title amber Build Script

call "C:\Program Files\Microsoft Visual Studio\2022\Community\VC\Auxiliary\Build\vcvarsall.bat" x64

cd /d "%~dp0"

echo Starting build for amber.sln...
...
msbuild "amber.sln" /p:Configuration=Release /p:Platform=x64 /m
...
if %errorlevel% neq 0 (
    ...
    if defined selectedName (
        ...
        if exist "release\amber.exe" (
            echo.
            echo Renaming amber.exe to !newName!...
            move "release\amber.exe" "release\!newName!" >nul 2>&1
            if exist "release\!newName!" (
                if not exist "release\amber.exe" (
                    echo Successfully renamed amber.exe to !newName!.
                ) else (
                    echo Warning: amber.exe still exists after rename attempt.
                )
            ) else (
                echo Failed to rename amber.exe.
            )
        ) else (
            echo Error: amber.exe does not exist in the release directory.
        )
    ) else (
